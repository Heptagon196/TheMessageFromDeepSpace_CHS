...
也可以双击游戏根目录的“汉化补丁配置.exe”，通过图形界面修改并校验配置。
配置工具使用 Windows 自带的 .NET Framework WinForms，不附带额外运行时。
默认按 F8 可在“仅译文”和“仅原文”模式之间即时切换；快捷键可在 INI 中修改。
默认按 F5 可重新读取四个翻译 JSON、INI 的 [Font]、[DialogueColors]、[Compatibility]、[PuzzleFixes]、题目及答案修正规则和中文字体并立即应用，无需重启游戏。
在回复框中依次输入“↑ ↑ ↓ ↓ ← → ← → B A”，可把当前题目的正确答案填入回复框；不会自动提交，可在 INI 的 [Cheats] 中关闭。
...
默认开启已知错误题目及答案的修正规则。规则位于 DeepSpaceChinese\Fix，以游戏界面显示题号命名；题面和答案集可单独修正，两者都提供时必须同时匹配才会原子替换。
...
题目及答案修正：
DeepSpaceChinese\Fix\数字.json               按游戏界面显示题号保存的修正规则，可选修改答案集
