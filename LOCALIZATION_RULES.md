...
- 中文 IME 开启后，场景共用输入框的物理回车可能被 TMP 误判为提交并触发失焦，而粘贴换行不受影响。兼容层只可在 `TMP_InputField.OnUpdateSelected` 处理键盘事件期间，对已跟踪且 `lineLimit == 0` 的 `InputTextDummy` 临时按 `MultiLineNewline` 解释，调用结束（含异常路径）立即恢复原始 backing field；禁止永久写入 `lineType` 或 `lineLimit`。清除自定义输入验证器时必须先清 `inputValidator`，再把 `characterValidation` 设为 `None`，避免 TMP setter 把验证模式反向改回 `CustomValidator`。
...
配置编辑器固定使用 .NET Framework 4.7.2 WinForms，依赖 Windows 已安装的 .NET Framework，不打包 WebView、Chromium 或独立运行时。编辑器直接读写同目录的 `DeepSpaceChinese.ini`，提供常规开关、快捷键、七种角色颜色和字体来源设置；保存时校验配置，并保留未知行及原注释。
