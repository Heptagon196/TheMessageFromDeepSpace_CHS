...
- `TermLogger.Configure()` 会把不带尾随空格的 `NAME SIGNAL` 与负数信号编号直接拼接成 `NAME SIGNAL-42`。必须用无空格模板 `NAME SIGNAL{DYN_0}` 翻译为“为信号 {DYN_0} 命名”，并用负数编号做回归测试；不能只依赖预制体字段的静态译文，也不能凭视觉习惯在模板中擅自补空格。
